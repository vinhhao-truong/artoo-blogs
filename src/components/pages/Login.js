...
const Login = () => {
  ...
  const handleSubmitLogin = async (e) => {
    ...
    try{
      ...
      console.log(data)
      dispatch(login({idToken: data.idToken, uid: data.localId}));
      dispatch(initiateProfile(JSON.parse(localStorage.getItem(`AB(${data.localId})`)).myProfile));
      dispatch(initiateMyBlogs(JSON.parse(localStorage.getItem(`AB(${data.localId})`)).myBlogs))
      navigate("/");
